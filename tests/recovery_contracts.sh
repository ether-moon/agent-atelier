...
assert_contains "$EXECUTE_SKILL" "### Startup Resume Sweep (Run Once After Team Spawn)" \
  "execute skill defines startup resume sweep"
assert_contains "$EXECUTE_SKILL" 'cold-resume: owner session unavailable' \
  "execute skill reclaims stranded implementing work on cold resume"
assert_contains "$EXECUTE_SKILL" 'Create watchdog recovery job.' \
  "execute skill documents watchdog recovery cron creation"
assert_contains "$EXECUTE_SKILL" 'Create monitor poll job.' \
  "execute skill documents monitor poll cron creation"
...
assert_contains "$RECOVERY_PROTOCOL" 'Do not separately invoke `/agent-atelier:monitors spawn` after calling `/agent-atelier:execute`.' \
  "recovery protocol assigns runtime restoration to execute skill"
...
assert_contains "$RECOVERY_SPEC" 'start `/agent-atelier:execute`, which recreates the monitor poll cron, the watchdog recovery cron, and one startup resume sweep over the recovered state' \
  "recovery spec routes cold resume through execute skill"
...
assert_contains "$CLI_SURFACE" 'creates the `*/15` watchdog recovery cron job' \
  "cli surface assigns watchdog recovery cron ownership to execute"

assert_contains "$SESSION_LIMIT_RETRY" 'start `/agent-atelier:execute`, which recreates monitors and both cron jobs' \
  "session-limit design delegates full restart recovery to execute"
